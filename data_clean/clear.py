...
from functions import (
    DropDatasetDuplicate,
    DropDatasetDuplicate_SimHash,
...
def merge_dataset_as_single_file(
    groups_cnt: int = 50000, max_len: int = 512, min_len: int = 3, cut_max_len: bool = False
) -> None:
    """
    将多个数据集合并为一个数据集
    """
    ...
    log.info(
        "merge into file: {}, 全部数据共{}行，清洗后剩余{}行".format(save_file, all_cnt, keep_cnt), save_to_file=True
    )
...
def remove_dataset_duplicate_rows(groups_cnt: int = 50000) -> None:
    ...
    if len(cur_rows) > 0:
        ...
        write_single_parquet_file(save_file, df)
    log.info(
        "merge into file: {}, 全部数据共{}行，文档去重后剩余{}行".format(save_file, all_cnt, keep_cnt),
        save_to_file=True,
    )
...
def read_and_write_template_baike(
